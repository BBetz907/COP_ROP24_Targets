...
nigeria_targets <- targets_and_msd |> filter(country_name == "Nigeria",
...
nigeria_targets_long <- nigeria_targets |>  
  mutate(indicator = factor(indicator, levels = indicator_order),
         indicator = if_else(standardized_disaggregate=="KeyPop/Status", str_c(indicator, 
                                                                               ...
                                                                               sep="-"), indicator)) |> 
  group_by(psnu, fiscal_year, indicator, key_pops, msd_version) |> 
  summarise(targets = sum(targets, na.rm=TRUE),
            cumulative = sum(cumulative, na.rm=TRUE),
  ) |>   
    arrange(key_pops, psnu, indicator, fiscal_year) |> print()
...
nigeria_results_targets <- nigeria_targets_wide |> 
  pivot_longer(cols = 5:last_col(), names_to = c("results_or_targets", "fy"), names_sep = "_") |> 
  filter(!is.na(value)) 

nigeria_kp_prev_by_pop <- nigeria_results_targets 
# |> filter(indicator=="KP_PREV")
write_csv(nigeria_kp_prev_by_pop, "Data Out/nigeria_kp_prev_by_pop.csv")
